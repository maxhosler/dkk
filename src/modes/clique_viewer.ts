...
export class CliqueViewer implements IMode
{
    ...
    readonly brick_canvas: DAGCanvas; //JRB

    readonly POLYTOPECANVAS: Boolean //JRB
    readonly BRICKCANVAS: Boolean //JRB
    ...
    {
        this.dag = dag;
	this.POLYTOPECANVAS = false;
	this.BRICKCANVAS = true;
        ...
        //Resize
        if(this.polytope.dim > 3 && this.POLYTOPECANVAS)
        {
    ...
    */
	...
	let clique_hasse=this.cliques.hasse;
	...
	if (this.draw_options.draw_compat_edges())
	{
			...
			if (this.cliques.bricks_compatible(i,j))
			{
	                ctx.draw_line(
	                    positions[i],
			    positions[j],
			    this.draw_options.compat_edge_color(),
	...
	{   
	    ...
	    this.draw_mini_brick(
	        pos,
		i,
	        this.cliques.clique_from_bricks([i]),
	        data,
	        ctx
	    );
	}
...
//       	return brick_layout_rows
//            .map(v => v.transform([[scale,0],[0,scale]));
